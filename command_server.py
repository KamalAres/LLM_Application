...
class AICommandExecutorServer:
    ...
    def generate_command(self, user_request: str) -> str:
        """
        Generate a Windows command based on user request
                ...
                messages = [
    {
        'role': 'system',
        'content': (
            "You are a Windows CMD command generator. ONLY output valid Windows CMD commands.\n"
            "NEVER use Linux/Unix commands like grep, cat, ls, pwd, etc.\n"
            "Return ONLY the raw command, no explanations or formatting.\n"
            "Windows CMD Examples:\n"
            "- List files: dir\n"
            "- Change directory: cd foldername\n"
            "- Go up directory: cd ..\n"
            "- Display file: type filename.txt\n"
            "- Find text in file: findstr \"text\" filename.txt\n"
            "- View IP config: ipconfig\n"
            "- See processes: tasklist\n"
    ...
    },
    {
        ...
        'content': user_request
            ...
            # Handle pipe commands and convert to Windows
            if '|' in command:
                parts = command.split('|')
                converted_parts = []
                for part in parts:
                    ...
                    for linux_cmd, windows_cmd in linux_to_windows.items():
                        if part.startswith(linux_cmd):
                            part = part.replace(linux_cmd, windows_cmd, 1)
                            break
                    converted_parts.append(part)
                command = ' | '.join(converted_parts)
            else:
                # Convert single commands
                for linux_cmd, windows_cmd in linux_to_windows.items():
                    if command.startswith(linux_cmd):
                        command = command.replace(linux_cmd, windows_cmd, 1)
                        break
            
            # Clean up Windows commands
